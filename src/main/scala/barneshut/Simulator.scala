...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val boundariesNew = new Boundaries
    boundariesNew.maxX = math.max(boundaries.maxX, body.x)
    boundariesNew.maxY = math.max(boundaries.maxY, body.y)
    boundariesNew.minX = math.min(boundaries.minX, body.x)
    boundariesNew.minY = math.min(boundaries.minY, body.y)
    boundaries
  }
